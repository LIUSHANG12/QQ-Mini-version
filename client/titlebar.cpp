...
void TitleBar::initControl()
{
	...
	mylayout->setContentsMargins(5, 0, 0, 0); // 设置布局间隙，左边间隙为5，其它设为0
	mylayout->setSpacing(0);                  // 设置布局里部件间的间隙

	// 设置标题栏的垂直策略，水平方向优先扩展，垂直方向固定
	m_pTitleContent->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
	setFixedHeight(TITLE_HEIGHT);             // 给标题栏设置固定高度
...
void TitleBar::paintEvent(QPaintEvent* event)
{
	...
	painter.setRenderHint(QPainter::SmoothPixmapTransform, true); // 设置像素转化为平滑转化

	// 当窗口最大化或还原后，窗口长度改变，标题栏相应的做出改变
	// parentWidget()返回父部件
...
void TitleBar::mousePressEvent(QMouseEvent* event)
{
	if (m_buttonType == MIN_MAX_BUTTON)
	{
		...
		{
			m_isPressed = true;
			m_startMovePos = event->globalPos(); //globalPos()返回事件发生时鼠标坐标在的全局位置
		}
	}
	else
	{
...
void TitleBar::onButtonRestoreClicked()
{
	...
	emit signalButtonResoreClicked();
}
