...
void TalkWindowShell::initControl()
{
	...
	QList<int> leftWidgeSize;
	leftWidgeSize << 154 << width() - 154;            
	ui.splitter->setSizes(leftWidgeSize);             // 分类器设置尺寸
...
{
	m_tcpClientSocket = new QTcpSocket(this);                // 构造套接字
	m_tcpClientSocket->connectToHost("127.0.0.1", gtcpPort); // 连接
}
...
文件数据包个数:群聊标志+发信息员工QQ号+收信息员工QQ号（群QQ号）+信息类型（2）+文件字节数+bytes+文件名+data_begin+文件数据
...
群聊文本信息如：1100012001100005World 表示QQ10001向群2001发送文本信息"World"
单聊图片信息如：0100011000201images066		  表示QQ10001向QQ10002发送1个表情66.png
群聊文件信息如：1100052000210bytesTest.txtdata_beginhelloworld
									  表示QQ10005向群2000发送文件信息，文件是Test.txt，文件内容长度为10，内容为"helloworld"

群聊文本信息解析： 1 10001 2001 1 00005 World
单聊图片信息解析： 0 10001 10002 01 images 066
...
void TalkWindowShell::processPendingData()
{
	...
	while (m_udpReceiver->hasPendingDatagrams())
	{
		...
		if (btData[0] == '1')                                // 群聊
		{
			...
			QChar cMsgType = (QChar)btData[groupFlagWidth + employeeWidth + groupWidth];
			if (cMsgType == '1')                             // 文本信息
			{
				msgType = 1;
			...
			else if (cMsgType == '2')                        // 文件信息
			{
				...
				int poseData_begin = strData.indexOf("data_begin");

				// 文件名称
				QString fileName = strData.mid(posBytes + bytesWidth, poseData_begin - posBytes - bytesWidth);
				gfileName = fileName;                       

				// 文件内容
				int dataLengthWidth;
				int posData = poseData_begin + QString("data_begin").length();
		...
		else   // 单聊
		{
			...
			// 获取信息的类型
			QChar cMsgType = (QChar)btData[groupFlagWidth + employeeWidth + employeeWidth];
			if (cMsgType == '1')               // 文本信息
			{
				msgType = 1;
